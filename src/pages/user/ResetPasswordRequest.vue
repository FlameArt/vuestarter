...
div
  ...
  .mt-5
  .bg-green-300.px-5.py-1(v-if="state.isGoodResult") Письмо со ссылкой на сброс пароля успешно отправлено на 
    span.font-bold {{ state.email }}
    div Должно прийти в течении 30 минут. Если не приходит, проверьте папку Спам
  .flex.bg-red-300.px-5.py-1(v-if="state.errors.length > 0") {{ state.errors.join(". ") }}
